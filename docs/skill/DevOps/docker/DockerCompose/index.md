...
> docker-compose文件：/data/docker/nginx/docker-compose.yml
...
#### 创建挂载目录
> 本地的挂载目录：/data/docker/mysql

需要创建的挂载目录如下：
- /data/docker/mysql/log
...
- /etc/mysql/conf.d/my.cnf ——> ./conf.d
- https://raw.githubusercontent.com/eggert/tz/master/asia ——> localtime

##### my.cnf
``` sh
cp goofy_borg:/mysql/conf.d/my.cnf ./conf.d
```

::: warning 提示
如果启动不起来，就docker logs看一下，应该是root密码没设置
:::
...
###### [client]配置模块 ######
[client]
# 设置客户端默认字符集为utf8mb4，支持更多的字符（包括emoji）
default-character-set=utf8mb4

# 指定Unix套接字文件路径，用于本地连接
socket=/var/lib/mysql/mysql.sock

###### [mysql]配置模块 ######
[mysql]
# 设置MySQL客户端默认字符集为utf8mb4，确保客户端与服务器之间的通信使用一致的字符集
default-character-set=utf8mb4
...
# MySQL进程运行的用户，默认是mysql
user=mysql

# 设置SQL模式，剔除ONLY_FULL_GROUP_BY以避免分组查询问题
# STRICT_TRANS_TABLES: 严格模式，对无效或丢失的数据值进行严格检查
# NO_ZERO_IN_DATE: 禁止插入日期字段中的'0000-00-00'
# NO_ZERO_DATE: 禁止插入日期字段中的'0000-00-00'
# ERROR_FOR_DIVISION_BY_ZERO: 在除数为零时返回错误而不是警告
# NO_ENGINE_SUBSTITUTION: 禁止在不支持指定存储引擎时自动替换为默认存储引擎
...
# 设置MySQL8的密码认证插件为mysql_native_password，以便低版本Navicat等工具能够连接
default_authentication_plugin=mysql_native_password

# 禁用符号链接以防止各种安全风险，如目录遍历攻击
symbolic-links=0

# 允许的最大并发连接数，默认是151，这里设置为1000以支持更多连接
max_connections=1000

# 设置服务端使用的字符集为utf8mb4，支持更多的字符（包括emoji）
character-set-server=utf8mb4

# 创建新表时将使用的默认存储引擎，默认是InnoDB
default-storage-engine=INNODB

# 表名的大小写敏感性：
# 0: 表名按指定方式存储，并且比较区分大小写（Linux默认）
# 1: 表名以小写形式存储在磁盘上，比较不区分大小写（Windows默认）
lower_case_table_names=0

# 设置允许的最大数据包大小，默认是16M，可以根据需要调整
...
# InnoDB 设置
innodb_file_per_table=1  # 为每个表创建单独的.ibd文件
innodb_flush_method=O_DIRECT  # 刷新方法，推荐使用O_DIRECT
innodb_doublewrite=ON  # 双写缓冲区功能开关

# 备份和恢复
explicit_defaults_for_timestamp=1  # 明确处理时间戳字段
```
...
> docker-compose文件：/data/docker/mysql/docker-compose.yml
...
services:
  mysql:
    ...
    volumes:
      # 映射日志目录
      - E:/data/docker/mysql/log:/var/log/mysql:rw
      # 映射数据目录
      - E:/data/docker/mysql/data:/var/lib/mysql:rw
      # 映射配置目录
      - E:/data/docker/mysql/conf.d:/etc/mysql/conf.d:rw
