...
用本站已启用的 Mermaid 即可，不另装引擎，也不为架构单独加顶栏。拓扑 / 部署 / 模块图统一用 ` ```mermaid ` + `flowchart`，并在图内写 `theme: base` 与黑白灰 `themeVariables`（与 [家庭网络](/ops/network/) 同款）；勿用 ` ```architecture `（易渲染失败）。图会随正文宽度缩放。

| 类型 | 位置 | 说明 |
|------|------|------|
| 家庭工作室网络拓扑 | [家庭网络](/ops/network/) | 与 [设备资产](/assets/)、[快捷入口](/shortcuts/) 交叉链接 |
| 项目部署 / 运行依赖 | [运行环境](/environment/) 下对应项目页 | 如芋道、WordPress 环境页的「部署架构」；项目可再分子页 **开发 / 测试 / 生产**（如 [芋道](/environment/yudao/)），节标题对齐「服务器（SSH）」「数据库」「应用访问」等访问信息 |
| 项目模块 / 分层 | 知识库开发文档对应介绍页 | 如 [芋道](/dev/framework/yudao/) |
